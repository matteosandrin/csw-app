...
#import "CSWMealsViewController.h"

@interface CSWMealsViewController ()

@end

@implementation CSWMealsViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell setupWithTitle:[(NSString*)meals[indexPath.row][@"Description"] stringByReplacingOccurrencesOfString:@"Menu for " withString:@""]];
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!isLoading) {
        ...
        [[CSWManager sharedManager] getMealPdfFileWithUrl:meal[@"DownloadUrl"]
...
-(void) refresh {
    ...
    [[CSWManager sharedManager] getMealsListWithCompletionBlock:^(NSArray *array) {
        ...
        if (array.count > 0) {
            ...
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.tableView.frame.size.width, 50)];
            [label setText:@"No meals available"];
            [label setFont:[UIFont sanFranciscoFontWithSize:12]];
            [label setTextColor:[UIColor darkCSWBlueColor]];
            [label setBackgroundColor:[UIColor whiteColor]];
            [label setTextAlignment:NSTextAlignmentCenter];
